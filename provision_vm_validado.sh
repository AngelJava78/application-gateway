...
az network nsg rule create --resource-group $rgName --nsg-name $nsgName --name Allow-HTTPS --protocol Tcp --direction Inbound --priority 120 --source-address-prefixes '*' --source-port-ranges '*' --destination-port-ranges 443 --access Allow
...
# Asociar NSG directamente a la NIC
nicName=$(az vm show --resource-group $rgName --name $vmName --query "networkProfile.networkInterfaces[0].id" --output tsv | awk -F '/' '{print $NF}')
az network nic update --resource-group $rgName --name $nicName --network-security-group $nsgName
